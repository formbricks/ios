...
final class SurveyManager {
    ...
    func track(_ action: String, completion: (() -> Void)? = nil, hiddenFields: [String: Any]? = nil) {
        ...
        service.getEnvironmentState { [weak self] result in
            ...
            case .success(let response):
                ...
                if let responseData = response.data {
                    self.startRefreshTimer(expiresAt: responseData.expiresAt)
                }
                self.filterSurveys()
                if (isInitial) {
            ...
            case .failure:
                if (isTried == false) {
                    self.refreshEnvironmentIfNeeded(force: force, isInitial: isInitial, isTried: true)
                } else {
